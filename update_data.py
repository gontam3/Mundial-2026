...
def run(excel_path, out_path="data.json"):
    ...
    scored_matches = sorted([int(k) for k,v in official.items() if v[1] is not None])
    print(f"  Resultados oficiales: {len(scored_matches)}/72")
    ...
    for p in PLAYERS:
        ...
        # Match history for evolution chart
        match_history=[]
        cumulative=0
        for mid_int in scored_matches:
            mid=str(mid_int)
        ...
        print(f"  {p}: {total} pts (pos+bonus+equipos: {extra})")
    ...
    data={"groups":GROUPS,"group_order":GROUP_ORDER,"official":official,
          ...
          "scored_matches":scored_matches,"players":players}
    with open(out_path,"w",encoding="utf-8") as f:
        json.dump(data,f,ensure_ascii=False,separators=(",",":"))
    print(f"\n✅ data.json — {len(scored_matches)}/72 partidos")
